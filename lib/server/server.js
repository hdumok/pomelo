...
var Server = function (app, opts) {
  ...
  //自己的组件直接调用app.event监听，不想外部组件需要写成一定格式才能监听到app.event上的东西
 ...
 * Handle request
 */
//msgRemote 里的 forwardMessage
//this.app.components.__server__ 的 handler
//components的server handler
//server/server.js 里的handler
...
var doForward = function(app, msg, session, routeRecord, cb) {
  ...
  try {
    ...
    app.sysrpc[routeRecord.serverType].msgRemote.forwardMessage(
      session,
      msg,
      session.export(), //session导出的是一份备份， 对方收到的第二个参数
