...
var SessionService = function(opts) {
  ...
  this.sessions = {};     // sid -> session , 这个sid是 session的id  不是server的id
...
SessionService.prototype.bind = function(sid, uid, cb) {
  ...
  //如果配置了每个用户在本进程只能有一个session，并且该session已经存在，那就不能重复绑定session到该uid
  ...
  for(var i=0, l=sessions.length; i<l; i++) {
    // 如果进程 该 uid对应的某个session  与 本次调用想绑定的session有了 那就直接下一步
...
SessionService.prototype.getByUid = function(uid) {
  return this.uidMap[uid]; //这里是可以拿到一个数组的session的
};
...
var Session = function(sid, frontendId, socket, service) {
  ...
  // private
  this.__socket__ = socket; //真实的socket的备份
...
Session.prototype.closed = function(reason) {
  ...
  this.emit('closed', this.toFrontendSession(), reason); //session 的 closed信号
...
//前端session的 push 其实就是真实session的 import,连缓存都省了，只是换个名字
