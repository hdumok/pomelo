...
var Component = function(app, opts) {
  opts = opts || {};
  this.app = app;
  this.connector = getConnector(app, opts); //当前前端进程对子进程的协议的connnector
  this.encode = opts.encode;
  this.decode = opts.decode;
  this.useCrypto = opts.useCrypto;
  this.useHostFilter = opts.useHostFilter; //IP防火墙开关
  this.useAsyncCoder = opts.useAsyncCoder; //同步解码函数
  this.blacklistFun = opts.blacklistFun; //黑名单判断函数
  this.keys = {};
  this.blacklist = [];
  ...
  //每个connector实例维护一个connection总数，实际上是个connection组件，内部提供连接的存储对象和维护函数
...
var bindEvents = function(self, socket) {
  ...
  //连接删除的时候，连接 和 用户连接缓存都清理，但是用户解绑时连接其实存在的
...
var getSession = function(self, socket) {
  ...
  //给原生的socket断连 配置上session销毁自带session的操作
  socket.on('disconnect', session.closed.bind(session)); //收到socket的断开信号，主动断开连接的 this.emit('closed', this.toFrontendSession(), reason);
  socket.on('error', session.closed.bind(session));

  //原生session模块自清理后放出来的信号
  session.on('closed', onSessionClose.bind(null, app)); //app.event.emit(events.CLOSE_SESSION, session);
  ...
  //解绑的时候，其实连接不删除，只是减少了登录用户数量，删除了登录用户的连接info  this.logined[uid]
...
var handleMessage = function(self, session, msg) {
  ...
  self.server.globalHandle(msg, session.toFrontendSession(), function(err, resp, opts) {
    ...
    self.send(msg.id, msg.route, resp, [session.id], opts, //opts参数要返回的内容
      function() {});
  });
};
