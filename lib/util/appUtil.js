...
/**
 * Initialize application configuration. app初始化 启动时处理传入的环境变量
 */
module.exports.defaultConfiguration = function(app) {
  var args = parseArgs(process.argv);
  setupEnv(app, args); //master给的启动变量里的环境变量 或者 环境变量, 必须第一步，后面的配置都根据环境变量来的
  loadMaster(app); //加载 '/config/master.json', 放到app.master
  loadServers(app); //加载 '/config/servers.json', 放到app.servers,serverMap[server.id] = server;
  processArgs(app, args);
  configLogger(app);
  loadLifecycle(app); //加载循环文件 lifecycle
};

/**
 * Start servers by type. 启动子进程，如过自己是父进程的话，否则直接返回
 */
...
var processArgs = function(app, args) {
  ...
  if (masterha === 'true') {
    ...
    app.set(Constants.RESERVED.CURRENT_SERVER, args, true); //普通进程
  } else {
    app.set(Constants.RESERVED.CURRENT_SERVER, app.getMaster(), true); //master进程
  }
};
