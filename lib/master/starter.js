 ...
 starter.runServers = function(app) {
  ...
  switch(condition) {
    ...
    for (var serverId in servers) {
      this.run(app, servers[serverId]); //这句是关键，启动服务器
    }
    break;
    default:
...
starter.run = function(app, server, cb) {
  ...
  if (utils.isLocal(server.host)) {
    ...
    starter.localrun(process.execPath, null, options, cb); //本地调用shell启动子进程
  } else {
    ...
    starter.sshrun(cmd, server.host, cb);  //远程调用shell启动子进程
  }
};
